...
int conti;
int multi;
long double a = -0.026015469;
long double b = -0.13514401;
long double c = 0.32375830;
long double d = -0.10163618;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="rass-cgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Control Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = (.22*rand()/RAND_MAX)*10 + 0.5;		//random x between 0.5-2.7
	//printf ("x %Lf \n",x);
        y = 0.2*rand()/RAND_MAX;			//random y between 0 and .20
	//printf ("y %Lf \n",y);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to rass-cgrfs.dat \n");
 return 0;
}
