...
int conti;
int multi;
long double a = 0.020980978;
long double b = 0.073069578;
long double c = 0.056399424;
long double d = -0.035831687;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="rass-mgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Maser Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
        x = (.3*rand()/RAND_MAX)*10 - 0.3;		//random x between 0 and 3
	//printf ("x %Lf \n",x);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to rass-mgrfs.dat \n");
 return 0;
}
