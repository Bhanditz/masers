...
int conti;
int multi;
long double a = 0.22046256;
long double b = -0.63305124;
long double c = 0.53677342;
long double d = -0.12302010;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="int-cgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Control Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = (.15*rand()/RAND_MAX)*10 + 1;		//random x between 0.5-2.7
	//printf ("x %Lf \n",x);
        y = 0.13*rand()/RAND_MAX;			//random y between 0 and .10
	//printf ("y %Lf \n",y);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to int-cgrfs.dat \n");
 return 0;
}
