...
int conti;
int multi;
long double a = 1022.0836;
long double b = -73.492138;
long double c = 1.7596358;
long double d = -0.014028189;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="swift-lumsim-m.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = (.4*rand()/RAND_MAX)*10 + 40.5;	//random x between 40.5 and 44.5
	//printf ("x %Lf \n",x);
        y = 0.16*rand()/RAND_MAX;			//random y between 0 and .16
	//printf ("y %Lf \n",y);
