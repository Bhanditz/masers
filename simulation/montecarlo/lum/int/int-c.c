...
int conti;
int multi;
long double a = 2523.2298;
long double b = -178.48132;
long double c = 4.2058150;
long double d = -0.033015917;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="int-lumsim-c.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Control Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = (.25*rand()/RAND_MAX)*10 + 42;	//random x between 42 and 44.5
	//printf ("x %Lf \n",x);
        y = 0.12*rand()/RAND_MAX;			//random y between 0 and .12
	//printf ("y %Lf \n",y);
