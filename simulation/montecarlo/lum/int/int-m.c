...
int conti;
int multi;
long double a = -63.322674;
long double b = 0.63575378;
long double c = 0.073324697;
long double d = -0.0012510974;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="int-lumsim-m.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = (.15*rand()/RAND_MAX)*10 + 42;	//random x between 42 and 43.5
	//printf ("x %Lf \n",x);
        y = 0.17*rand()/RAND_MAX;			//random y between 0 and .17
	//printf ("y %Lf \n",y);
