...
int conti;
int multi;
long double a = 27.528080;
long double b = 7.6144799;
long double c = 0.69605276;
long double d = 0.020985180;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="rass-flux-cgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Control Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = -(.36*rand()/RAND_MAX)*10 - 9.8;		//random x between 0.5-2.7
	//printf ("x %Lf \n",x);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to rass-flux-cgrfs.dat \n\n");
 return 0;
}
