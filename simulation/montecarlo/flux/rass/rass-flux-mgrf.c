...
int conti;
int multi;
long double a = 74.140471;
long double b = 19.568467;
long double c = 1.7130351;
long double d = 0.049689343;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="rass-flux-mgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Maser Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = -(.3*rand()/RAND_MAX)*10 - 10.5;		//random x between 0.5-2.7
	//printf ("x %Lf \n",x);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to rass-flux-mgrfs.dat \n\n");
 return 0;
}
