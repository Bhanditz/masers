...
int conti;
int multi;
long double a = -247.20496;
long double b = -71.116261;
long double c = -6.7965178;
long double d = -0.21583537;
int n;

void counter(long double * array){
     FILE *fp;
     char output[]="int-flux-mgrfs.dat";
     int q;
...
int main() {
  printf ("Monte Carlo Data Generator for Maser Group Fluxes\n");
     for (i=0;i<1000000;i++){				//calculations per cycle
       x = -(.08*rand()/RAND_MAX)*10 - 9.5;		//random x between 0.5-2.7
	//printf ("x %Lf \n",x);
  ...
  fclose(p);*/
  counter(xs);
  printf ("\nThe data points have been saved to int-flux-mgrfs.dat \n\n");
 return 0;
}
